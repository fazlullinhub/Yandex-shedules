...
YaScheduleApp::YaScheduleApp(std::string openWeatherApiKey, std::string yaScheduleApiKey, std::string cityFrom, std::string cityTo) {
	this->openWeatherClient = new httplib::Client("https://api.openweathermap.org");
	this->yandexScheduleClient = new httplib::Client("https://api.rasp.yandex.net");
...
YaScheduleApp::getLatLon(std::string city)
{
...
std::vector <std::vector <std::map <std::string, std::string>>> YaScheduleApp::getScheduleBetweenCities() {
	...
	std::map <std::string, std::string>departureStationTitle;
	std::map <std::string, std::string>departureStationCode;
	std::map <std::string, std::string>arrivalStationTitle;
	std::map <std::string, std::string>arrivalStationCode;
	...
	for (int i = 0; i < result["segments"].size(); i++) {
		...
		departureStationTitle["Departure Station Title"] = result["segments"][i]["from"]["title"].get<std::string>();
		departureStationCode["Departure Station Code"] = result["segments"][i]["from"]["title"].get<std::string>();
		arrivalStationTitle["Arrival Station Title"] = result["segments"][i]["from"]["arrival_platform"].get<std::string>();
		arrivalStationCode["Arrival Station Title"] = result["segments"][i]["from"]["arrival_platform"].get<std::string>();
		transportType["Transport Type"] = result["segments"][i]["from"]["transport_type"].get<std::string>();
		departureDate["Departure Date"] = result["segments"][i]["from"]["departure"].get<std::string>();
		arrivalDate["Arrival Date"] = result["segments"][i]["from"]["arrival"].get<std::string>();
		codeTransportCompany["Code Transport Company"] = result["segments"][i]["from"]["code"].get<std::string>();
		race.push_back(raceTitle);
		race.push_back(raceNumber);
		race.push_back(departureStationTitle);
		race.push_back(departureStationCode);
		race.push_back(arrivalStationTitle);
		race.push_back(arrivalStationCode);
...
std::vector <std::vector <std::map <std::string, std::string>>> YaScheduleApp::getScheduleByTransport(std::string transport) {
	if (this->schedule.size() != 0) {
		return this->findScheduleByTransport(transport);
	}
	else {

		this->getScheduleBetweenCities();
		return this->findScheduleByTransport(transport);
	}
}
